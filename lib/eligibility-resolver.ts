...
import { eligibilityRegion, eligibilityUserPool, eligibilityClientId } from './eligibility-environment.ts';
...
import { readEligibilityDraft, getAuthorizedDraft } from './eligibility-draft.ts';
import type { Deliverable, Expert, KnowledgeChunk, KnowledgeDocument } from './types.ts';

const verifier = CognitoJwtVerifier.create({ userPoolId: eligibilityUserPool, clientId: eligibilityClientId, tokenUse: 'access' });
...
export async function resolveEligibilityContext(request: Request, input: { expertId: string; projectCode?: string; saCode: string; documentIds: string[]; historical?: boolean; loadReferenceChunks?: boolean; metadataOnly?: boolean }, authenticatedActor?: EligibilityActor) {
  ...
  const documents = await Promise.all(input.documentIds.map(async (id) => {
    if (id.startsWith('draft_')) {
      if (!input.metadataOnly) return readEligibilityDraft(id, expert);
      ...
      return { ...draft, originalFileName: draft.fileName } as unknown as Deliverable;
    }
    ...
    return input.metadataOnly ? document : readEligibilityOriginal(document);
  }));

...
export async function createEligibilityDraft(expert: Expert, actorId: string, input: { fileName: string; fileSize: number; declaredTitle: string; deliverableType: string; saCode: string }) {
  if (!expert.saCodes?.includes(input.saCode) || !/\.(pdf|docx|txt|md|csv|xlsx|xls)$/i.test(input.fileName)
    || !Number.isInteger(input.fileSize) || input.fileSize < 1 || input.fileSize > MAX_BYTES) {
    throw new EligibilityAccessError('Fisierul trebuie sa fie PDF, DOCX, text sau tabel, de cel mult 15 MB, pentru o SA permisa.', 422);
  }
...
export async function reviseEligibilityDraft(id: string, expert: Expert, input: { declaredTitle: string; deliverableType: string; saCode: string }) {
...
export async function completeEligibilityDraft(id: string, expert: Expert) {
  const draft = await getAuthorizedDraft(id, expert);
  ...
  const document = await readEligibilityOriginal({ ...draft, fileHash, originalFileName: draft.fileName } as unknown as Deliverable);
...
export async function getEligibilityDraftUrl(id: string, expert: Expert) {
  const draft = await getAuthorizedDraft(id, expert);

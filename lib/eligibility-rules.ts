...
export const executableRulesetSchema = z.object({
  schemaVersion: z.literal('eligibility-rules-v2'), projectCode: z.string().min(1),
...
export type RuleEvidence = {
  ...
  documents: Array<{ id: string; extractedText: string; analysisComplete: boolean }>;
...
export function criterionApplies(rule: ExecutableCriterion, context: Pick<RuleEvidence, 'projectCode' | 'roleId' | 'category' | 'saCode' | 'at'>) {
...
export const ELIGIBILITY_OPERATOR_HANDLERS: Record<ExecutableCriterion['operator'], Handler> = {
  semantic_evidence(rule, evidence) {
    if (rule.operator !== 'semantic_evidence') throw new Error('Handler incompatibil.');
    ...
    const validDocuments = answer.documentQuotes.filter((quote) => evidence.documents.some((document) => document.id === quote.documentId && contains(document.extractedText, quote.quote)));
    const relevant = rule.parameters.requiredTerms.every((term) =>
    ...
    if (!validSources.length || !validDocuments.length || !relevant || answer.explanation.trim().length < 20) {
...
export function selectTemporalRuleset(rows: AiEligibilityRuleset[], projectCode: string, at: string) {
  ...
  const candidates = rows.filter((row) => ['active', 'archived'].includes(row.status) && row.publishedAt
    && Date.parse(row.publishedAt) <= instant).flatMap((row) => {
